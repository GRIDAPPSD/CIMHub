# extract_transformer_impedance.py
# Robustly pulls TransformerMeshImpedance r/x (pu), maps to ends, and prints ohmic + pu values.
from rdflib import Graph, RDF, URIRef
from math import sqrt, atan2, degrees

CIM_FILE    = "simple_cim.rdf"  # adjust path if needed
SBASE_KVA   = 1000.0                  # change to your transformer kVA (e.g., 1500)
RATEDU_IS_LL = True                   # True if ratedU is line-to-line (12.47, 4.16). Set False if it's L-N.
...
# --- 1) Collect TransformerEnds ratedU ---
ends_ratedU = {}
q_rated = """
...
# If your ratedU is actually line-to-neutral, convert to L-L for base impedance calc
if not RATEDU_IS_LL:
    ends_ratedU = {k: (v * sqrt(3.0)) for k, v in ends_ratedU.items()}

# --- 2) Find all TransformerMeshImpedance nodes & pull r, x, from, to ---
meshes = []
...
if not meshes:
    print("No TransformerMeshImpedance objects found. Check your file/namespace.")
    raise SystemExit

# --- 3) Print results per mesh impedance on each end base ---
for m in meshes:
    r_pu = m.get("r_pu")
    x_pu = m.get("x_pu")
