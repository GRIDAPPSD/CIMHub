...
def ConvertMDB(cfg):
  ...
  ODBCstr = 'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=' + rootdir + '/' + mdbname + '.mdb'
  
  print ('ODBC Connection String:')
  print (' ', ODBCstr)
  con = pyodbc.connect(ODBCstr)
  ...
  try:
      ...
      print ('DevConductors table not found')
  ...
  for row in cursor.fetchall():
    ...
    lf.write('new line.' + name + ' bus1=' + bus1 + phsfx + ' bus2=' + bus2 + phsfx + \
             ' phases=' + str(phqty) + ' spacing=' + spacing + ' ' + wiretype + '=' + str(wirearray) + \
             ' length=' + '{:.4f}'.format(length) + ' units=mi' + '\n')
  sgf.close()
  lf.close()
  ...
  cursor.execute("SELECT * FROM  (InstPrimaryTransformers " + \
  ...
  cursor.execute("SELECT * FROM InstSection " +\
                "INNER JOIN InstCapacitors ON InstCapacitors.SectionId = Instsection.Sectionid")
      ...
      if kvar!=0.0:
          ...
          if row.PrimaryControlMode == 'VOLTS':
              ...
              capf.write(' type=voltage element=capacitor.' + name + ' terminal=1'+\
                  ...
                  ' delay=' + str(row.TimeDelaySec) + ' delayoff=' + str(row.TimeDelaySec)+ ' deadtime=0\n')
          else:
  ...
  # Query all load
  cursor.execute( "SELECT * FROM InstSection " + \
                  "INNER JOIN Loads ON InstSection.SectionId = Loads.SectionId")
  # Non-Split-Phase
  ...
  cursor.execute( "SELECT * FROM InstSection " + \
                  "INNER JOIN InstLargeCust ON InstSection.SectionId = InstLargeCust.SectionId")
  for row in cursor.fetchall():
    name = dss_name(row.UniqueDeviceId)
    bus2 = dss_name(row.ToNodeId)
    phqty = count_ph(row.SectionPhases)
  ...
  if Feeder == 1:
    ...
    cursor.execute(	"SELECT * FROM Node INNER JOIN InstSubstationTransformers " +\
