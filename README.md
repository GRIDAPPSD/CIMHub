...
Blazegraph requires Java 8, which is no longer widely available for new 
installations.  Blazegraph isn't compatible with Java 9 or newer versions.  
Therefore, we recommend using Blazegraph in a Docker container: 
...
3. From inside the Docker Terminal, run two example conversions of the IEEE 13-Bus example:
   * _cd example_
   * _./example.sh_
   * _tail sum*.csv_ to verify that the converted OpenDSS files ran correction
...
You can also run the IEEE 13-bus example conversions from the host.
...
you have GridLAB-D installed on the host.
...
In order to modify the CIMHub Java code, you will need to install Apache Maven and then use _mvn clean install_.
