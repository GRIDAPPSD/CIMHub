echo off
set DB_URL="http://localhost:9999/blazegraph/namespace/kb/sparql"
rem set CIMHUB_PATH="../cimhub/target/cimhub-1.0.4-jar-with-dependencies.jar"
set CIMHUB_PATH="../releases/cimhub-1.0.4.jar"
rem set CIMHUB_PATH="../cimhub/target/libs/*;../cimhub/target/cimhub-1.0.1-SNAPSHOT.jar"
set CIMHUB_PROG="gov.pnnl.gridappsd.cimhub.CIMImporter"

